...
#
# CorreÃ§Ãµes incluÃ­das (2026-02-07):
# - Problema de caracteres "DiagnÃƒÂ³stico": garantir arquivo em UTF-8 + header coding utf-8
...
# - SanitizaÃ§Ã£o forte (NaN/strings em numÃ©rico) antes do treino e antes de prever
...
class ToDenseTransformer(BaseEstimator, TransformerMixin):
    ...
    NecessÃ¡rio porque HistGradientBoostingRegressor exige entrada densa.
    """
    ...
    def transform(self, X):
        if hasattr(X, "toarray"):
            X = X.toarray()
        # reduz memÃ³ria e acelera um pouco
        try:
...
# -----------------------------
# MemÃ³ria de custos (persistÃªncia entre abas)
# -----------------------------
...
# -----------------------------------------
# SANITIZAÃ‡ÃƒO FORTE PARA O MODELO DE PREÃ‡O
# -----------------------------------------
...
def train_price_model(d: pd.DataFrame, min_samples: int = 40):
    ...
    # ðŸ”¥ FIX DO ERRO: converte sparse -> dense antes do model
...
def run_factory_optimizer(df_sim: pd.DataFrame, hours_avail: float = 40.0):
    """Sugere mix de produÃ§Ã£o por CLUSTER_NOME maximizando lucro total com limite de horas.
    ...
    if linprog is None:
        raise RuntimeError("SciPy nÃ£o estÃ¡ instalado (linprog indisponÃ­vel). Adicione 'scipy' ao requirements.txt.")
    ...
    # VariÃ¡veis: q_i >= 0 (quantidade por cluster)
    ...
    if not res.success:
        raise RuntimeError(f"OtimizaÃ§Ã£o falhou: {res.message}")
    ...
    # arredondamento amigÃ¡vel (sem perder o valor real)
...
# -----------------------------------------
# MATRIZ BCG (portfÃ³lio por cluster)
# -----------------------------------------
...
# --- Custos (persistentes) ---
st.sidebar.markdown("---")
st.sidebar.markdown("### ðŸ’° Custos (persistentes)")
cc = st.session_state["cost_config"]
cc["custo_hora"] = st.sidebar.number_input("Custo/hora mÃ¡quina (R$)", min_value=0.0, value=float(cc.get("custo_hora", 8.0)), step=0.5, key="cc_custo_hora")
...
if not df_enriched.empty:
    ...
    if isinstance(price_metrics, dict) and "ERROR" in price_metrics:
        ...
        st.sidebar.caption(f"Linhas treino: {price_metrics.get('TRAIN_ROWS','â€”')} | mÃ­nimo: {price_metrics.get('MIN_SAMPLES','â€”')}")
    else:
    ...
    with tab1:
        ...
        st.markdown("---")
        st.subheader("â˜ï¸ Nuvens de Palavras (VisÃ£o Geral)")
        ...
        sw_local.update(["de", "para", "3d", "pla", "com", "o", "a", "em", "do", "da", "kit", "un", "cm", "mm", "peÃ§as", "pecas", "und", "unid"])
        sw_local.update(extras)

        c_cloud1, c_cloud2 = st.columns(2)
        with c_cloud1:
            st.caption("ðŸ“Œ MAIS FREQUENTES (o que mais aparece nos tÃ­tulos)")
            ...
            try:
                ...
                st.warning(f"NÃ£o consegui gerar a nuvem: {e}")

        with c_cloud2:
            st.caption("ðŸ’° MAIOR VALOR (palavras associadas a preÃ§os mais altos)")
            try:
                word_prices = {}
                ...
                st.warning(f"NÃ£o consegui gerar a nuvem por valor: {e}")
    ...
    with tab3:
        ...
        excluir_wc = st.text_input("Palavras para excluir (separe por vÃ­rgula)", value="", key="wc_excluir_tab3")
        ...
        if isinstance(price_metrics, dict) and "MAE" in price_metrics:
            dD.metric("Modelo PreÃ§o", f"MAE {format_brl(price_metrics['MAE'])}")
        else:
    ...
    with tab8:
        ...
        st.caption(f"Itens vÃ¡lidos TOTAL pÃ³s-limpeza: {len(df_enriched)} | mÃ­nimo para treinar: {price_metrics.get('MIN_SAMPLES', 40) if isinstance(price_metrics, dict) else 40}")
    ...
    with tab11:
        ...
        st.dataframe(view[["score_rec", "CLUSTER_NOME", "FONTE", "PRODUTO", "PreÃ§o", "Logistica", "LINK"]],
                     hide_index=True, use_container_width=True)
    ...
    # 13. OTIMIZADOR
    with tab_otimizador:
        st.header("ðŸ—ï¸ Otimizador de FÃ¡brica (mix de produÃ§Ã£o)")
        st.caption("Usa matemÃ¡tica (linprog) para sugerir quantidades por cluster, maximizando lucro total com limite de horas semanais.")

        hours_avail = st.number_input("Horas disponÃ­veis na semana", min_value=1.0, value=40.0, step=1.0)
        ...
        if st.button("Rodar otimizador"):
            try:
                ...
                if plan.empty:
                    ...
                    st.download_button("â¬‡ï¸ Baixar plano (CSV)", data=csv, file_name="otimizador_plano.csv", mime="text/csv")
            except Exception as e:
                st.error(f"Otimizador indisponÃ­vel/erro: {e}")

    # 14. MATRIZ BCG
    with tab_bcg:
        st.header("ðŸ§© Matriz BCG (Volume x Margem)")
