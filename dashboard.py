...
def train_price_model(d: pd.DataFrame, min_samples: int = 120):
    """
    Modelo de preço compatível com matriz esparsa (TF-IDF + OneHot).
    Troca o HistGradientBoosting (não aceita sparse) por Ridge + log1p no target.
    """
    ...
    metrics = {"MAE": mae, "R2": r2, "MIN_SAMPLES": int(min_samples), "TRAIN_ROWS": int(len(data)), "MODEL": "Ridge(log1p)"}
    return pipe, metrics, data


def apply_price_model(def apply_price_model(model_pipe, d: pd.DataFrame):
...
price_model, price_metrics, df_enriched = train_price_model(df_enriched, min_samples=120)
